...
let contraintes_serveur ligne j ordre oc  =
  let n = ligne.nservers and flots = ligne.flows in
    begin
      ...
      let jj = (n+1-j)*(n+2-j)/2 in
	begin
	  for k = 0 to  (n-j) do
	    Printf.fprintf oc "t%d - t%d >=0; \n" (jj+k) (jj+k+1);
	    for l = k+1 to (n-j+1) do
	      begin
		for i = 1 to ligne.nflows do
		  ...
		  if (flots.(i-1).debut == j) then
		    genere_alpha i j (jj+k) (jj+l) flots.(i-1).alpha oc;
		done;
	      end;
	    done;
	  done;
	  for k=0 to (n-j) do
		...
		begin
		  Printf.fprintf oc "f%ds%dt%d = f%ds%dt%d;\n" i j 
		    (jj+k-n+j-1) i (j-1) (jj+k);
		  Printf.fprintf oc "t%d >= t%d;\n"  (jj+k-n+j-1) (jj+k);
		end;
	    done;
	    genere_beta ligne.nflows flots j (jj+k-n+j-1) (jj+n-j+1) 
	      ligne.servers.(j-1).beta oc;
	  done;
	end;
    end;;
